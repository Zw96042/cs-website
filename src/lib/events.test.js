...
test('keeps events scheduled for today in the club timezone', () => {
  const eventTitles = getUpcomingEvents(new Date('2026-09-01T04:59:59Z')).map(
    ({ title }) => title
  );
...
test('drops an event when the next club calendar day starts', () => {
  const eventTitles = getUpcomingEvents(new Date('2026-09-01T05:00:00Z')).map(
    ({ title }) => title
  );
...
test('returns no homepage events after every scheduled event has passed', () => {
  const eventTitles = getUpcomingEvents(new Date('2026-10-01T12:00:00Z')).map(
    ({ title }) => title
  );
